...
void APVZ_USFX_LAB02GameMode::BeginPlay()
{
	...
	for (int32 i = 0; i < NumberZombieMago; ++i)
	{
		// Cambia la siguiente línea para crear instancias de AZombieMago
		AZombieMago* NewZombieMago = GetWorld()->SpawnActor<AZombieMago>(AZombieMago::StaticClass(), SpawnLocation);

		if (NewZombieMago)
		{
			NewZombieMago->SetSpawnAfter(FMath::RandRange(1, 10));
			NewZombieMago->SetActorHiddenInGame(true);
			NewZombieMago->SetActorEnableCollision(true);
			NewZombieMago->SetCanMove(false);
			ArrayZombies.Add(NewZombieMago);
		}
	}
